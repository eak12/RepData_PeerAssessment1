...
```{r histogramSteps, fig.width=14, message=FALSE}
...
```{r mean&median, message=FALSE, warning=FALSE}
...
The mean and median total number of steps taken are **`r mean`** and **`r median`**, respectively.
...
```
The maximum number of steps occured between the `r y` to `r x` interval.
...
```{r impute, message=FALSE, results="hide"}
library(impute)
...
```{r histogramStepsImpute, fig.width=14, message=FALSE}
...
```{r mean&medianMissing, message=FALSE, warning=FALSE}
...
When I replaced missing values, the mean and median total number of steps taken are **`r meanMissing`** and **`r medianMissing`**, respectively.  When I replaced missing values - the mean is lower than when I did not replace missing values.
